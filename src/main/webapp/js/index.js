...
$('nav .nav_down').mouseenter(function () {
    ...
    $(this).children('img').attr('src', 'http://localhost:8080/videosys/img/nav_down2.png');
});
$('nav .nav_down').mouseleave(function () {
    $('#nav_down').stop().slideUp();
    $(this).children('img').attr('src', 'http://localhost:8080/videosys/img/nav_down.png');
});
       ...
       // alert(params);
        $.post("../user/validateEmail",params,function(data){
            if(data===0){
               ...
               $("#emailMsg").text("该邮箱已注册").css("color","red");
            }
        });
    }
...
function commitLogin(){
   ...
   if(null!=email && email!="" && null!=password && password!=""){
        ...
        // alert(params);
        // post要小写
        $.post("../user/login.do",params,function(data){
        // alert(data);
                 if(data === 1){
                      //登录框消失
                      $("#login").addClass("hidden");
